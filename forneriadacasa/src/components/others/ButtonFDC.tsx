...
export default function ButtonFDC({ go, href, typeBtn, label, styleClass, onClick, title }: ButtonFDCProps) {
  ...
  return (
    ...
    <>
      ...
      } title={title} className={`btn-fdc flex items-center justify-center flex-row bg-[var(--button-color)] hover:bg-[var(--button-color-hover)] active:bg-[var(--button-color-hover)] text-white text-base font-bold h-fit w-fit !py-2 !px-3 hover:shadow-[inset_0_-3px_0_var(--button-color)] active:shadow-[inset_0_-3px_0_var(--button-color)] rounded-md cursor-pointer ${styleClass ?? ""}`}>{label}</Link>
      ) : typeBtn ? (
        <button type={typeBtn || "button"} onClick={onClick} title={title} className={`btn-fdc flex items-center justify-center flex-row bg-[var(--button-color)] hover:bg-[var(--button-color-hover)] active:bg-[var(--button-color-hover)] text-white text-base font-bold h-fit w-fit !py-2 !px-3 hover:shadow-[inset_0_-3px_0_var(--button-color)] active:shadow-[inset_0_-3px_0_var(--button-color)] rounded-md cursor-pointer ${styleClass ?? ""}`}>{label}</button>        
      ) : (
        <a href={href} target="_blank" rel="noopener noreferrer" onClick={onClick} title={title} className={`btn-fdc flex items-center justify-center flex-row bg-[var(--button-color)] hover:bg-[var(--button-color-hover)] active:bg-[var(--button-color-hover)] text-white text-base font-bold h-fit w-fit !py-2 !px-3 hover:shadow-[inset_0_-3px_0_var(--button-color)] active:shadow-[inset_0_-3px_0_var(--button-color)] rounded-md cursor-pointer ${styleClass ?? ""}`}>{label}</a>
      )}
    </>
